...
function todoReducer(state: State[], action: Action) {
    switch (action.type) {
        ...
        default:
            throw new Error(`type을 알 수 없음`);
    }
}

type TodoDispatch = Dispatch<Action>;
const TodoStateCotext = createContext<State[]|null>(null);
...
export function TodoProvider({children}:Children) {
    const [state, dispatch] = useReducer(todoReducer, initialTodos);
    const nextId = useRef(5);

    return(
        <TodoStateCotext.Provider value={state}>
            <TodoDispatchContext.Provider value={dispatch}>
                <TodoNextIdContext.Provider value={nextId}>
                    {children}
                </TodoNextIdContext.Provider>
            </TodoDispatchContext.Provider>
        </TodoStateCotext.Provider>
    );
}

export function useTodoState() {
    const context = useContext(TodoStateCotext);
    if (!context) {
